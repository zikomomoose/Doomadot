...
async function fetchRssItems(url) {
  const r=await fetch(url,{headers:{"User-Agent":"Dumadot/2.0"},signal:AbortSignal.timeout(12000)});
  if(!r.ok) throw new Error(`RSS ${r.status}`);
  const xml=await r.text();
  const decode=s=>s.replace(/<!\[CDATA\[([\s\S]*?)\]\]>/g,'$1').replace(/&amp;/g,'&').replace(/&quot;/g,'"').replace(/&#39;/g,"'").replace(/&lt;/g,'<').replace(/&gt;/g,'>');
  return [...xml.matchAll(/<item>([\s\S]*?)<\/item>/gi)].map(m=>{const b=m[1];const get=t=>decode((b.match(new RegExp(`<${t}[^>]*>([\s\S]*?)<\/${t}>`,'i'))||[])[1]||'').trim();return {title:get('title'),link:get('link'),description:get('description')};}).filter(x=>x.title&&x.link);
}
const FEEDS = [
...
async function collectResearch() {
  ...
  for (const [source,url] of FEEDS) {
    ...
    } catch (e) { console.error(`Research feed failed: ${source}: ${e.message}`); }
  }
...
async function generatePost(requestedTopic = "", researchContext = "", requestedFormat = "") {
...
async function createDraft(topic="", opts={}) {
  const post=await generatePost(topic,opts.researchContext||"",opts.format||"");
...
async function publishPost(id) {
  ...
  const linkedinId=await publishToLinkedIn(post.content);
...
setInterval(async()=>{
  ...
  if(weekday!==0 && weekday!==6 && local.hour===ph && local.minute===pm && !db.prepare("SELECT 1 FROM scheduler_runs WHERE run_date=?").get(local.date)){
    try{
      ...
      if(!plan){ await createWeeklyPlan(); plan=db.prepare("SELECT * FROM content_plans WHERE plan_date=?").get(local.date); }
      const draft=await createDraft(plan?.topic||"",{format:plan?.format||""});
  ...
  // Per-post scheduling is also governed by the same master auto-publish switch.
  if(setting("auto_publish")==="true"){
...
app.post("/api/research/refresh",async(req,res)=>{try{const n=await collectResearch();res.json({ok:true,count:n});}catch(e){res.status(500).json({error:e.message});}});
...
app.post("/api/plans/generate",async(req,res)=>{try{const items=await createWeeklyPlan();res.json({ok:true,items});}catch(e){res.status(500).json({error:e.message});}});
app.post("/api/plans/:id/generate",async(req,res)=>{try{const p=db.prepare("SELECT * FROM content_plans WHERE id=?").get(Number(req.params.id));if(!p)throw new Error("Plan item not found.");const d=await createDraft(p.topic,{format:p.format});db.prepare("UPDATE content_plans SET status='draft',post_id=? WHERE id=?").run(d.id,p.id);res.json(d);}catch(e){res.status(500).json({error:e.message});}});

app.get("/api/analytics",(req,res)=>{const posts=db.prepare(`SELECT p.*,COALESCE(m.impressions,0) impressions,COALESCE(m.reactions,0) reactions,COALESCE(m.comments,0) comments,COALESCE(m.reposts,0) reposts,COALESCE(m.clicks,0) clicks FROM posts p LEFT JOIN post_metrics m ON m.post_id=p.id WHERE p.status='published' ORDER BY p.published_at DESC`).all();const totals=posts.reduce((a,p)=>{a.impressions+=p.impressions;a.engagements+=p.reactions+p.comments+p.reposts;a.clicks+=p.clicks;return a},{impressions:0,engagements:0,clicks:0});const byPillar={};for(const p of posts){const k=p.pillar||"Unclassified";byPillar[k]??={posts:0,impressions:0,engagements:0};byPillar[k].posts++;byPillar[k].impressions+=p.impressions;byPillar[k].engagements+=p.reactions+p.comments+p.reposts;}res.json({posts,totals,byPillar});});
